...
class BlogScraper:
    ...
    def _extract_links_with_category_navigation(
        self, selenium_scraper, base_url: str, site_conf: dict
    ) -> set:
        """Extract blog post links by navigating through category pages."""
        links = set()

        # First, get category links from the main blog page
        category_selectors = site_conf.get(
            "category_selectors", ['a[href*="/category/"]', 'a[href*="/tag/"]']
        )
        logger.info(f"Extracting category links with selectors: {category_selectors}")

        category_links = selenium_scraper.extract_links_with_selenium(
            base_url, category_selectors
        )
        ...
        for category_url in filtered_categories[
            :5
        ]:  # Limit to first 5 categories for performance
            try:
                logger.info(f"Extracting links from category: {category_url}")
                post_selectors = site_conf.get(
                    "post_selectors",
                    ['a[href*="/blog/"]', "article a", ".post-title a"],
                )

                category_links = selenium_scraper.extract_links_with_selenium(
                    category_url, post_selectors
                )
